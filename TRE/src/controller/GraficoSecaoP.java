...
public class GraficoSecaoP {

    private DefaultCategoryDataset barra;
    private ChartPanel painel;
    private JFreeChart grafico;

    public GraficoSecaoP() {
        new Dados().start();
    }

    public ChartPanel jfreechar() {
        barra = new DefaultCategoryDataset();
        grafico = ChartFactory.createBarChart3D("Terminal Regional Eletoral- Seções pelo RN - Prefeito", "Urnas", "Votos", barra, PlotOrientation.VERTICAL, true, true, false);
                ...
                try {
                    ...
                    for (int i = 0; i < ListSecao.size(); i++) {
                        for (int j = 0; j < ListSecao.get(i).getCandidatos().size(); j++) {
                            if (ListSecao.get(i).getCandidatos().get(j).getCargo().equals("Prefeito")) {
                    ...
                    Logger.getLogger(GraficoSecaoP.class.getName()).log(Level.SEVERE, null, ex);
                }
