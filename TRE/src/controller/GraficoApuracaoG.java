...
public class GraficoApuracaoG {

    private DefaultCategoryDataset barra;
    private ChartPanel painel;
    private JFreeChart grafico;

    public GraficoApuracaoG() {
        new Dados().start();
    }

    public ChartPanel jfreechar() {
        barra = new DefaultCategoryDataset();
        grafico = ChartFactory.createBarChart3D("Terminal Regional Eletoral- Apuração pelo RN- Governador", "Urnas", "Votos", barra, PlotOrientation.VERTICAL, true, true, false);
            ...
            while (true) {
                ...
                try {
                    ...
                    for (int i = 0; i < ListCandidatos.size(); i++) {
                        if (ListCandidatos.get(i).getCargo().equals("Governador")) {
                    ...
                    Logger.getLogger(GraficoApuracaoG.class.getName()).log(Level.SEVERE, null, ex);
                }
